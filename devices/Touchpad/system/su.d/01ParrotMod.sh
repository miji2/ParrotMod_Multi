...
if $bb test -e "/sys/block/zram0"; then # enable zram if supported - it should be 1/4 the device ram
	...
	#***CHANGE_ME***: change No. of cores below to match your device
	write /sys/block/zram0/max_comp_streams 1 # on 2015 Google devices, this is half the number of cores
	#***CHANGE_ME***: change zRAM size below to match your device
	write /sys/block/zram0/disksize $((256*1024*1024)) # it should be 1/4 the device ram
